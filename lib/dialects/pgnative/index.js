...
class Client_PgNative extends Client_PG {
  ...
  // pg-native (libpq, text protocol) can't bind a Buffer through the `?::bytea`
  // cast batchUpdate's union source uses ("invalid input syntax for type bytea"),
  // unlike node-postgres. Fall back to per-row updates for binary, where the
  // Buffer binds in assignment context with no cast.
