...
class QueryCompiler_Redshift extends QueryCompiler_PG {
  ...
  // Same UPDATE ... FROM form (and Postgres casts, inherited) as Postgres, but
  // Redshift has no RETURNING.
  _batchUpdateUnion() {
    return this._batchUpdateFromSelect({ allowReturning: false });
  }

  // Redshift has no jsonb_to_recordset, so reject 'json' instead of inheriting
  // the Postgres form that would fail at runtime.
  _batchUpdateJson() {
    throw new Error(`batchUpdate mode 'json' is not supported on redshift`);
  }

  // Redshift inherits the Postgres value casts but lacks the bytea and jsonb
  // types, so fail loud instead of emitting a cast Redshift rejects at runtime.
  _batchUpdateColumnCasts(allColumns) {
    const casts = super._batchUpdateColumnCasts(allColumns);
    const unsupported = casts.find(
      (cast) => cast === 'bytea' || cast === 'jsonb'
    );
    if (unsupported) {
      throw new Error(
        `batchUpdate: Redshift has no ${unsupported} type; pass an explicit columnTypes cast`
      );
    }
    return casts;
  }
