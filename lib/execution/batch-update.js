...
function batchUpdate(client, tableName, batch, key = 'id', options = {}) {
  ...
  // Binary (Buffer) handling. 'json' serializes the chunk through JSON.stringify,
  // which mangles a Buffer into { type: 'Buffer', data: [...] } — so binary can't
  // ride json mode; fail loud rather than corrupt the value. Some dialects also
  // can't carry a Buffer in the set-based statement (Oracle writes a large BLOB
  // through a RETURNING ... INTO LOB, which a single MERGE can't express), so
  // those fall back to per-row updates that reuse the dialect's normal blob path.
  const hasBinary = rows.some((row) =>
    Object.values(row).some((value) => Buffer.isBuffer(value))
  );
  if (hasBinary && mode === 'json') {
    throw new TypeError(
      `batchUpdate mode 'json' does not support binary (Buffer) values; use mode 'union' or 'case'`
    );
  }
  const perRowForBinary =
    hasBinary && !client.client.batchUpdateSupportsBinary();
  // Each strategy binds a different number of parameters per row, so cap the
  // chunk to the dialect's per-statement limit (mssql: 2098) accordingly:
  //  - union: one value per cell -> keys + columns
  //  - case:  each column's CASE matches every row -> columns*(keys+1), plus the
  //           WHERE keys per row
  //  - json:  the whole chunk is a single JSON parameter -> independent of rows
  ...
  const maxRowsByBindParameters =
    perRowBindParameters > 0
      ...
      : Infinity;
  // Some dialects also cap the rows a single statement can carry structurally,
  // independent of parameters (e.g. SQLite's compound-SELECT term limit for
  // union mode); take the tighter of the two ceilings.
  ...
  const chunks = chunk(rows, Math.max(1, Math.min(chunkSize, maxRowsPerChunk)));
