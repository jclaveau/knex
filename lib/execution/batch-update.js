  ...
  return Object.assign(
    Promise.resolve().then(async () => {
      ...
      return runInTransaction(async (tr) => {
        ...
        // Set-based UPDATE(s) for one column subset, chunked for the mode.
        const applySetBased = (cols, statementMode, ret) =>
          runChunks(
            tr,
            tableName,
            chunkRowsFor(
        ...
        const applyBinaryColumns = (cols) =>
          canBindBinarySetBased
            ? applySetBased(cols, 'union', undefined)
        ...
        if (mode === 'json' && hasBinary) {
          ...
          if (jsonableColumns.length) {
            results.push(...(await applySetBased(jsonableColumns, 'json')));
          }
        ...
        return applySetBased(columns, mode, returning);
      });
    }),
    {
