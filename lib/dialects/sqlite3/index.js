...
class Client_SQLite3 extends Client {
  ...
  // union mode emits one `UNION ALL SELECT` term per row; SQLite caps a compound
  // SELECT at SQLITE_MAX_COMPOUND_SELECT (default 500), so a chunk over 500 rows
  // fails to compile ("too many terms in compound SELECT") well before the bind
  // cap. case/json don't build a compound SELECT, so only the bind cap applies.
  batchUpdateRowLimit(mode) {
    return mode === 'union' ? 500 : Infinity;
  }
}
...
Object.assign(Client_SQLite3.prototype, {
  ...
  // SQLITE_MAX_VARIABLE_NUMBER, 32766 since SQLite 3.32 (better-sqlite3
  // inherits this); batchUpdate caps chunks to stay under it.
