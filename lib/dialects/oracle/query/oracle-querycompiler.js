...
class QueryCompiler_Oracle extends QueryCompiler {
  ...
  _batchUpdateUnion() {
    ...
    const sql = `merge into ?? tgt using ${source.sql} on (${onSql}) when matched then update set ${setSql}`;
  ...
  _batchUpdateJson() {
    ...
    const sql = `merge into ?? tgt using (select * from json_table(?, '$[*]' columns (${colDefsSql}))) ?? on (${onSql}) when matched then update set ${setSql}`;
    return this._batchUpdateRaw(sql, [
      table,
      JSON.stringify(rows),
      ...allColumns,
