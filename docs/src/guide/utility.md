...
- Large batches are split into chunks of `chunkSize`, each binding one parameter per cell (`rows × columns`). SQL Server caps a statement at 2100 parameters, so on MSSQL the effective chunk is automatically reduced to fit (`floor(2100 / columns)`) regardless of `chunkSize`. Other dialects have far higher limits (Postgres 65535, SQLite ≥3.32 32766) — tune `chunkSize` yourself if a single row is very wide.
