...
class Client_Oracledb extends Client_Oracle {
  ...
  // Oracle writes a BLOB through a RETURNING ... INTO a LOB locator, which the
  // set-based MERGE that batchUpdate emits can't express, so batchUpdate falls
  // back to per-row updates (which use that LOB path) when binary is present.
