      ...
      request.on('row', (row) => {
        stream.write(
          row.reduce((prev, curr) => {
            prev[curr.metadata.colName] = curr.value;
...
Object.assign(Client_MSSQL.prototype, {
  ...
  // SQL Server caps a single RPC at 2100 bound parameters; tedious sends queries
  // through sp_executesql, which itself consumes two of those slots (the
  // statement text and the parameter-definition string), leaving 2098 for the
  // query. batchUpdate reads this to size its chunks so a wide table can't
  // overflow (a 2100-parameter chunk is rejected — see the benchmark).
