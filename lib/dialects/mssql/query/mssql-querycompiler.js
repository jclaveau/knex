...
class QueryCompiler_MSSQL extends QueryCompiler {
  ...
  // MSSQL supports the base UPDATE ... FROM (SELECT ...) form; append the
  // affected-row count the same way standardUpdate does.
  _batchUpdateUnion() {
    return this._batchUpdateAppendRowcount(super._batchUpdateUnion());
  }

  // 'case' mode is a plain single-table UPDATE; still needs the rowcount tail.
  _batchUpdateCase() {
    return this._batchUpdateAppendRowcount(super._batchUpdateCase());
  }

  // 'json' strategy: expand one JSON parameter with OPENJSON ... WITH (typed
  // columns), then the base UPDATE ... FROM form. OPENJSON needs an explicit type
  // and a literal path per column. Rowcount tail like the other MSSQL forms.
  _batchUpdateJson() {
    ...
    const colDefsSql = allColumns
      .map(
        (column, index) =>
          `?? ${types[index]} ${this._batchUpdateJsonPath(column)}`
      )
      .join(', ');
    const setSql = columns.map(() => '?? = ??').join(', ');
    const setBindings = [];
    for (const column of columns) {
      setBindings.push(column, `src.${column}`);
    }
    const whereSql = keyColumns.map(() => '?? = ??').join(' and ');
    const whereBindings = [];
    for (const keyColumn of keyColumns) {
      whereBindings.push(`${table}.${keyColumn}`, `src.${keyColumn}`);
    }
    const sql = `update ?? set ${setSql} from openjson(?) with (${colDefsSql}) as ?? where ${whereSql}`;
    const compiled = this._batchUpdateRaw(sql, [
      table,
      ...setBindings,
      JSON.stringify(rows),
      ...allColumns,
    ...
    return this._batchUpdateAppendRowcount(compiled);
  }

  _batchUpdateAppendRowcount({ sql, bindings }) {
    return {
      sql: sql + this._returning('rowcount', '@@rowcount'),
      bindings,
      returning: '@@rowcount',
    };
  }
