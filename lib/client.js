...
class Client extends EventEmitter {
  ...
  // Rows a single batchUpdate statement may carry for a given mode, beyond the
  // bind-parameter cap (`maxBindParameters`). Some dialects have a structural
  // ceiling the parameter count doesn't capture — e.g. SQLite limits a compound
  // SELECT (union mode) to SQLITE_MAX_COMPOUND_SELECT terms. Infinity means only
  // the bind-parameter cap applies; dialects with such a limit override this.
  batchUpdateRowLimit() {
    return Infinity;
  }

  // Whether a batchUpdate may carry binary (Buffer) values in its set-based
  // statement. Most drivers bind a Buffer to the native binary type directly;
  // dialects that can't (Oracle routes a BLOB through RETURNING ... INTO a LOB,
  // which a single MERGE can't express) return false and fall back to per-row
  // updates that reuse the normal blob path.
  batchUpdateSupportsBinary() {
    return true;
  }
