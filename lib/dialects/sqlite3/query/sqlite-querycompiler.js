...
class QueryCompiler_SQLite3 extends QueryCompiler {
  ...
  // json_each + json_extract into a FROM source (SQLite >= 3.33 has UPDATE ...
  // FROM). SQLite is dynamically typed, so no column-type list is needed.
  ...
  _batchUpdateJson() {
    ...
    const selectSql = allColumns
      .map(() => 'json_extract(value, ?) as ??')
      .join(', ');
    const selectBindings = [];
    for (const column of allColumns) {
      selectBindings.push(`$.${column}`, column);
    }
    ...
    const sql = `update ?? set ${setSql} from (select ${selectSql} from json_each(?)) as ?? where ${whereSql}`;
