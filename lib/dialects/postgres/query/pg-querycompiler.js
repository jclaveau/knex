...
class QueryCompiler_PG extends QueryCompiler {
  ...
  // Postgres needs the values-source columns cast (bound params are typed
  // `unknown`), and supports RETURNING on UPDATE ... FROM.
  _batchUpdateUnion() {
    return this._batchUpdateFromSelect({ allowReturning: true });
  }

  // 'json' strategy: the whole chunk rides as a single jsonb parameter, expanded
  // by jsonb_to_recordset whose column-definition list gives exact types — so no
  // per-row params (sidesteps the bind limit) and no `unknown`-cast guessing.
  // Values must be JSON-serializable (no Buffer; bigint must be a string).
  _batchUpdateJson() {
    ...
    // jsonb_to_recordset needs a type for every column; reuse the cast inference
    // (caller columnTypes win, else inferred, else text).
    const types = this._batchUpdateColumnCasts(allColumns);
    const colDefsSql = allColumns
      .map((column, index) => `?? ${types[index]}`)
      .join(', ');
    ...
    let sql = `update ?? set ${setSql} from jsonb_to_recordset(?) as ??(${colDefsSql}) where ${whereSql}`;
    const bindings = [
      table,
      ...setBindings,
      JSON.stringify(rows),
  ...
  // Per-column cast types for the batchUpdate values source. A caller-supplied
  // `columnTypes` entry wins; otherwise infer from the JS value (covers
  // number/bigint/boolean/Date/Buffer/json — uuid, enum, arrays, and all-null
  // columns can't be inferred and need an explicit columnTypes entry).
  _batchUpdateColumnCasts(allColumns) {
    const { rows, columnTypes } = this.single.batchUpdate;
