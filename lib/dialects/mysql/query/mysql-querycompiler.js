...
class QueryCompiler_MySQL extends QueryCompiler {
  ...
  // MySQL/MariaDB have no UPDATE ... FROM, so correlate via a join:
  //   UPDATE t JOIN (SELECT ...) src ON t.k = src.k SET t.c = src.c
  _batchUpdateUnion() {
    ...
    const source = this._batchUpdateSource('src', {
      useAs: true,
      fromDual: false,
    });
    ...
    for (const keyColumn of keyColumns) {
      onBindings.push(`${table}.${keyColumn}`, `src.${keyColumn}`);
    }
    const setSql = columns.map(() => '?? = ??').join(', ');
    const setBindings = [];
    for (const column of columns) {
      setBindings.push(`${table}.${column}`, `src.${column}`);
    }
    const sql = `update ?? inner join ${source.sql} on ${onSql} set ${setSql}`;
  ...
  // 'json' strategy: the chunk rides as one JSON parameter, expanded by
  // JSON_TABLE into a derived table joined the same way the union form joins its
  // SELECT. JSON_TABLE needs an explicit type and a literal path per column.
  // Values must be JSON-serializable (no Buffer; bigint as string).
  _batchUpdateJson() {
    if (this.single.returning) {
      this._batchUpdateRejectReturning();
    }
    ...
    const sql = `update ?? inner join json_table(?, '$[*]' columns (${colDefsSql})) as ?? on ${onSql} set ${setSql}`;
    return this._batchUpdateRaw(sql, [
      table,
      JSON.stringify(rows),
      ...allColumns,
