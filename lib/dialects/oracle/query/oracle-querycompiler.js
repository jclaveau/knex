...
class QueryCompiler_Oracle extends QueryCompiler {
  ...
  _batchUpdateJson() {
    ...
    // The chunk rides as one VARCHAR2 bind; Oracle binds a string over 4000
    // bytes as LONG, which JSON_TABLE rejects (ORA-01461) — and knex's `?` path
    // can't bind it as a CLOB. Fail loud with the alternative rather than let the
    // driver surface a cryptic error deep in execution.
    const json = JSON.stringify(rows);
    const jsonBytes = Buffer.byteLength(json);
    if (jsonBytes > 4000) {
      throw new Error(
        `batchUpdate mode 'json' on oracle: the chunk's JSON payload (${jsonBytes} bytes) exceeds Oracle's 4000-byte bind limit (ORA-01461; CLOB binding is not supported yet). Use mode 'union' or 'case', or a chunkSize small enough to keep each chunk under 4000 bytes.`
      );
    }
    ...
    const sql = `merge into ?? ?? using (select * from json_table(?, '$[*]' columns (${colDefsSql}))) ?? on (${onSql}) when matched then update set ${setSql}`;
    return this._batchUpdateRaw(sql, [
      table,
      'tgt',
      json,
