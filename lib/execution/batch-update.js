...
function batchUpdate(
  ...
  return Object.assign(
    Promise.resolve().then(async () => {
      ...
      // Collapse duplicate keys (last wins) or throw — see dedupeByKey for why.
...
// Collapses the batch to one row per key (or throws on a repeat).
//
// This is REQUIRED, not just an optimization: a set-based UPDATE
// (UPDATE ... FROM / JOIN / MERGE) applies the matched set in no defined order,
// so the database cannot honor "the last row wins" for a duplicated key — it
// picks an arbitrary matching source row (Postgres/MySQL/MSSQL) or errors
// (Oracle MERGE, ORA-30926). Row order only exists here in the JS array, so
// deduping here — keeping the last occurrence — is what actually makes
// last-write-wins deterministic and gives every emitted statement unique keys.
