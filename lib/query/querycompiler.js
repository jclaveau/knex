...
class QueryCompiler {
  ...
  // Compiles a batch update (one set-based statement updating many rows with
  // different per-row values). Dispatches on the chosen strategy `mode`:
  //  - 'union' (default): UPDATE ... FROM (SELECT ... UNION ALL ...) / JOIN /
  //    MERGE — per-dialect (`_batchUpdateUnion`).
  //  - 'case':  UPDATE ... SET c = CASE WHEN key THEN ? ... — universal.
  //  - 'json':  expand one JSON param to a rowset — per-dialect (`_batchUpdateJson`).
  // Payload is set by the builder's `batchUpdate()` and fed by `knex.batchUpdate`.
  batchUpdate() {
    const mode = this.single.batchUpdate.mode || 'union';
    if (mode === 'case') {
      return this._batchUpdateCase();
    }
    if (mode === 'json') {
      return this._batchUpdateJson();
    }
    return this._batchUpdateUnion();
  }

  // Default 'union' strategy; MySQL (JOIN) and Oracle (MERGE) override it.
  _batchUpdateUnion() {
    return this._batchUpdateFromSelect({ allowReturning: false });
  }

  // 'json' strategy is dialect-specific (jsonb_to_recordset / OPENJSON /
  // JSON_TABLE / json_each); dialects that support it override this.
  _batchUpdateJson() {
    throw new Error(
      `batchUpdate mode 'json' is not supported on ${this.client.dialect}`
    );
  }

  // 'case' strategy: a single UPDATE whose every SET column is a CASE keyed on
  // the row's key, with the matched rows selected by an OR-expanded WHERE (no
  // row-value IN, so it is portable to every dialect). Values sit in assignment
  // context, so Postgres types them from the target column — no casts needed.
  _batchUpdateCase() {
    const table = this.single.table;
    const { rows, keyColumns, columns } = this.single.batchUpdate;
    if (this.single.returning) {
      throw new Error(
        `.batchUpdate().returning() is only supported with mode 'union'`
      );
    }
    // "?? = ?" per key column, AND-joined and parenthesised for a composite key.
    const keyMatch = (row) => {
      const sql = keyColumns.map(() => '?? = ?').join(' and ');
      const bindings = [];
      for (const keyColumn of keyColumns) {
        bindings.push(`${table}.${keyColumn}`, row[keyColumn]);
      }
      return {
        sql: keyColumns.length > 1 ? `(${sql})` : sql,
        bindings,
      };
    };
    const setParts = [];
    ...
    for (const column of columns) {
      // The SET target must be a bare column (Postgres/SQLite/Oracle reject a
      // table-qualified assignment target); the CASE/WHERE conditions may be
      // qualified.
      let caseSql = '?? = case';
      setBindings.push(column);
      for (const row of rows) {
        const match = keyMatch(row);
        caseSql += ` when ${match.sql} then ?`;
        setBindings.push(...match.bindings, row[column]);
      }
      // `else ??` keeps the current value for any row the WHERE matched but the
      // CASE didn't (none, given uniform shape — defensive, and ragged-ready).
      caseSql += ' else ?? end';
      setBindings.push(column);
      setParts.push(caseSql);
    }
    // Single key: an IN list. A composite key falls back to OR-expanded tuples
    // because row-value IN ((?,?), ...) isn't portable (MSSQL rejects it). The IN
    // list also stays flat — an OR chain parses into a row-deep expression tree
    // that trips SQLite's SQLITE_MAX_EXPR_DEPTH on large chunks.
    let whereSql;
    const whereBindings = [];
    if (keyColumns.length === 1) {
      const keyColumn = keyColumns[0];
      whereSql = `?? in (${rows.map(() => '?').join(', ')})`;
      whereBindings.push(`${table}.${keyColumn}`);
      for (const row of rows) {
        whereBindings.push(row[keyColumn]);
      }
    } else {
      const whereParts = [];
      for (const row of rows) {
        const match = keyMatch(row);
        whereParts.push(match.sql);
        whereBindings.push(...match.bindings);
      }
      whereSql = whereParts.join(' or ');
    }
    const sql = `update ?? set ${setParts.join(', ')} where ${whereSql}`;
    return this._batchUpdateRaw(sql, [table, ...setBindings, ...whereBindings]);
  }

  // UPDATE t SET c = v.c FROM (SELECT ... UNION ALL ...) v WHERE t.k = v.k
  _batchUpdateFromSelect({ allowReturning }) {
    const table = this.single.table;
    ...
    const whereBindings = [];
    for (const keyColumn of keyColumns) {
      whereBindings.push(`${table}.${keyColumn}`, `v.${keyColumn}`);
    }
    ...
    if (returning) {
      ...
      // (e.g. "id"), so a bare RETURNING reference is ambiguous.
      bindings.push(...returns.map((column) => `${table}.${column}`));
    }
    const compiled = this._batchUpdateRaw(sql, bindings);
    // Carry `returning` so the dialect response processor yields the rows
    // (without it the query resolves to an affected-row count).
    if (returning) {
      compiled.returning = returning;
    }
    return compiled;
  }

  _batchUpdateRejectReturning() {
    throw new Error(
      `.batchUpdate().returning() is not supported for ${this.client.dialect}`
    );
  }

  // `(SELECT v1 [as] k1, ... [from dual] UNION ALL SELECT v2, ...) [as] alias`.
  // Only the first SELECT names (and, where the dialect casts, casts) the
  // columns; later rows inherit them. Shared by every dialect's batchUpdate form.
  _batchUpdateSource(alias, { useAs, fromDual }) {
    const { rows, keyColumns, columns } = this.single.batchUpdate;
    const allColumns = [...keyColumns, ...columns];
    const castTypes = this._batchUpdateColumnCasts(allColumns);
    let sql = '';
    const bindings = [];
  ...
  // Per-column cast types for the values source, aligned to `allColumns`, or
  // null for no casts. Postgres (where bound params are typed `unknown`)
  // overrides this; other dialects don't need casts and have no columnTypes.
  _batchUpdateColumnCasts() {
    if (this.single.batchUpdate.columnTypes) {
      throw new Error(
        `batchUpdate columnTypes is only supported on the postgres family, not ${this.client.dialect}`
      );
    }
    return null;
  }

  // Per-column types for a 'json'-mode statement on a dialect whose JSON-rowset
  // function needs an explicit type per column (MySQL/MariaDB JSON_TABLE, MSSQL
  // OPENJSON, Oracle JSON_TABLE) — unlike Postgres (jsonb_to_recordset infers
  // from the value) and SQLite (typeless). Requires an explicit columnTypes map;
  // a length-bearing JS-value inference isn't safe to guess here.
  _batchUpdateJsonColumnTypes(allColumns) {
    const { columnTypes } = this.single.batchUpdate;
    if (!columnTypes || typeof columnTypes !== 'object') {
      throw new Error(
        `batchUpdate mode 'json' on ${this.client.dialect} needs an explicit columnTypes map (e.g. { name: 'varchar(255)' }); types can't be inferred for this dialect`
      );
    }
    return allColumns.map((column) => {
      ...
      if (!type) {
        throw new Error(
          `batchUpdate mode 'json' on ${this.client.dialect}: missing columnTypes entry for column "${column}"`
        );
      }
      return type;
    });
  ...
  // A column's JSON path, inlined as a literal — JSON_TABLE / OPENJSON reject a
  // bound parameter for the path. Restricted to a plain identifier name so the
  // literal never needs quote-escaping.
  _batchUpdateJsonPath(column) {
    if (!/^[a-z_][a-z0-9_]*$/i.test(column)) {
      throw new Error(
        `batchUpdate mode 'json' on ${this.client.dialect}: column "${column}" is not a plain identifier and can't be expressed as a JSON path`
      );
    }
    return `'$.${column}'`;
  }

  // Compile the raw ?? (identifier) / ? (value) template to dialect SQL.
  _batchUpdateRaw(sql, bindings) {
    const compiled = this.client.raw(sql, bindings).toSQL();
    return { sql: compiled.sql, bindings: compiled.bindings };
  }
