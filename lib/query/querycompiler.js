...
class QueryCompiler {
  ...
  // different per-row values). The default uses `UPDATE ... FROM (SELECT ...)`,
  // which standard SQL / SQLite / Postgres / MSSQL support; MySQL (no UPDATE
  // FROM) and Oracle (MERGE) override this. Payload is set by the builder's
  // `batchUpdate()` and fed by the `knex.batchUpdate(...)` utility.
  batchUpdate() {
    return this._batchUpdateFromSelect({ cast: false, allowReturning: false });
  }

  // UPDATE t SET c = v.c FROM (SELECT ... UNION ALL ...) v WHERE t.k = v.k
  _batchUpdateFromSelect({ cast, allowReturning }) {
  ...
  // Only the first SELECT names (and, when `cast`, casts) the columns; later
  // rows inherit them. Shared by every dialect's batchUpdate form.
  _batchUpdateSource(alias, { useAs, fromDual, cast }) {
    const { rows, keyColumns, columns, columnTypes } = this.single.batchUpdate;
    const allColumns = [...keyColumns, ...columns];
    // Per-column: caller-supplied `columnTypes` wins, else infer from the JS
    // value (needed for the types the heuristic can't see — uuid, enum, arrays,
    // all-null columns). Only the cast dialects (Postgres family) use this.
    const castTypes = cast
      ? allColumns.map(
          (column) =>
            (columnTypes && columnTypes[column]) ||
            this._batchUpdatePgCast(rows, column)
        )
      : null;
    let sql = '';
    const bindings = [];
  ...
  // Postgres types VALUES/param columns as `unknown`; the first SELECT casts
  // each column from the JS value type so comparisons and assignments resolve.
  _batchUpdatePgCast(rows, column) {
