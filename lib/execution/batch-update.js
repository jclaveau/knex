...
function batchUpdate(client, tableName, batch, key = 'id', options = {}) {
  ...
  // rows sharing a key — keep the last (default) or throw. mode: the SQL
  // strategy — 'union' (default), 'case', or 'json'.
  const {
    chunkSize = 1000,
    onDuplicateKey = 'last',
    columnTypes,
  ...
  // chunk to the dialect's per-statement limit (mssql: 2100) accordingly:
